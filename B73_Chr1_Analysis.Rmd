...
TE_Coords_Chr1 <- filter(TE_Coords, V1==1)
Output_TE$Chr1 <- Output_TE$Sequence %in% TE_Coords_Chr1$V4

Chromosome_One <- filter(Output_TE, Chr1 == TRUE)
Chromosome_One <- filter(Output_TE, TE_Type != "SoloLTR")
```
...
Reduced_TE_Data <- filter(Chromosome_One, TE_Type == "LTR" | TE_Type == "TIR" | TE_Type == "Helitron")

TIR_DATA <- filter(Reduced_TE_Data, TE_Type == "TIR")

LTR_Candidacy <- ggplot(data=Reduced_TE_Data, aes(x = TE_Type,fill = LTR_Candidacy)) + geom_bar()+ scale_fill_manual(values = cbbPalette2) + coord_flip() + theme_light()
LTR_Candidacy
#ggsave("B73_LTR_Candidacy.png")

Frequency_Table <- Reduced_TE_Data %>% group_by(TE_Type, LTR_Candidacy, ) %>% dplyr::summarise(Freq=n())
...
Freq_Table_PassFail <- Reduced_TE_Data %>% group_by(Test_Result, Nested_Host) %>% dplyr::summarise(Freq=n())
Freq_Table_PassFail
